...
import { Card, CardState } from './models/Card.js';
import { Player, Position } from './models/Player.js';
...
export class Board {
    ...
    private readonly grid: Array<Array<Card | null>>; // null represents removed card
    private readonly players: Map<string, Player>;
    private readonly waitingForCard: Map<string, Array<Deferred<void>>>; // position key -> waiting queue
    private readonly changeListeners: Array<Deferred<void>>; // listeners waiting for board changes
    ...
    public async map(f: (card: string) => Promise<string>): Promise<void> {
        ...
        const cardPositions = new Map<string, Array<Position>>();
     ...
     */
    public async watchForChange(): Promise<void> {
     ...
     */
    private async handlePreviousTurnCleanup(player: Player): Promise<void> {
        if (!player.hasFirstCard()) {
     ...
     */
    private turnDownCardIfPossible(position: Position): void {
     ...
     */
    private async flipFirstCard(player: Player, row: number, column: number, card: Card): Promise<void> {
     ...
     */
    private async flipSecondCard(player: Player, row: number, column: number, card: Card): Promise<void> {
